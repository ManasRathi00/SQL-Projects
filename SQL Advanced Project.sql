...
select * from IPL_team_players order by player_id;
--
...
/* HAVE USED TWO ATTEMPTS FOR THIS QUESTION.
ONE IS MORE RUDIMENTARY THAN THE OTHER BUT HAS APT LOGIC */

-- Attempt 1
select * from ipl_bidder_details; 
describe ipl_bidder_details;
select * from ipl_bidder_points;
describe ipl_bidder_points;
select * from ipl_bidding_details order by bidder_id;

create table temp_won(Bidder_id INT, Numberr_of_wins INT);
insert into temp_won select bidder_id , count(*) from ipl_bidding_details where bid_status like '%won%' group by bidder_id;
create table temp_total (bidder_id int, Number_of_bids INT);
insert into temp_total select bidder_id, count(*) from ipl_bidding_details where bidder_id != 102 group by bidder_id;
-- AS VISIBLE HERE I CREATE TWO TEMPORARY TABLES TO EXTRACT THE DATA TO MAKE IT EASIER TO MANIPULATE. I WAS HAVING TROUBLE DIVIDNG TWO AGGREGATE FUNCTIONS
-- select * from temp_total;
select b.bidder_id, c.bidder_name, a.number_of_bids, b.Numberr_of_wins, (b.numberr_of_wins/a.number_of_bids)*100 as percentage_of_wins
from temp_total a 
inner join temp_won b
ON b.bidder_id = a.bidder_id
inner join ipl_bidder_details c 
ON b.bidder_id = c.bidder_id;
-- THIS SELECT GETS THE EXPECTED OUTPUT ORDERED BY BIDDER_ID
drop table temp_won;
drop table temp_total;
-- THIS COMMAND DROPS THE TEMPORARY TABLES

-- Attempt 2
--  1) The percentage of wins of each bidder in the order of highest to lowest percentage.

select b.bidder_id, b.bidder_name, 
(select count(*) from ipl_bidding_details a where a.bid_status like '%won%' and a.bidder_id = b.bidder_id)/
(select no_of_bids from ipl_bidder_points c where c.bidder_id = b.bidder_id) * 100 as 'Percentage of Wins'
from ipl_bidder_details b;

-- Instead of saving data into tables, have directly did the mathematical function of dvision by using a nested select query as per DQL.



-- 2) The number of matches conducted at each stadium with stadium name, city from the database.
/*SELECT *, count(match_id) FROM IPL_MATCH_SCHEDULE group by stadium_id;
select * from ipl_stadium;*/

select a.stadium_id 'Stadium Id', b.stadium_name 'Stadium Name', b.city 'City', count(a.match_id) 'Match id'
from ipl_match_schedule a inner join ipl_stadium b 
on b.stadium_id = a.stadium_id
group by a.stadium_id
order by a.stadium_id;

-- Here have implemented a simple inner join to match the stadium id from two tables to get the desried outcome of the intersection of data.

-- Finding what is the percentage of wins by a team which has won the toss in a given stadium.
/*select * from ipl_stadium;
select * from ipl_match_schedule;
select * from ipl_match order by match_id;*/

-- ipl_stadium = a ,  ipl_match_schedule = b, ipl_match = c

select stadium_id 'Stadium Id', stadium_name'Stadium Name',
(select count(*) from ipl_match c join ipl_match_schedule b on c.match_id = b.match_id
where toss_winner = match_winner and (b.stadium_id = a.stadium_id)) / 
(select count(*) from ipl_match_schedule c where c.stadium_id = a.stadium_id) * 100 as 'Percentage of wins'
from ipl_stadium a;

/* Here have implemented a nested select query to join the outcomes of three tables (ipl_match,ipl_mtach_schedule and ipl_stadium)
and have used simple mathematical function of division on two aggregate functions of count() in order to get the 
desired outcome i.e. percentage of wins per stadium */


-- 4) The total bids along with bid team and team name.
select a.bid_team 'Bid Team', b.team_name'Team Name', count(a.bid_team) 'Total Bids'
from ipl_bidding_details a join ipl_team b 
...
-- Performed a simple join in order to get the desired result
...
select  match_id, match_winner 'Winner Team ID' , win_details 'Win Details' 
from ipl_match;

-- As requested, have displayed the team_id of the team that won the match. Although there might be a fault in the data of the table ipl_match as most winning team_ids are either 1 or 2.

-- 6) The total matches played, total matches won and total matches lost by team along with its team name.

/*select * from ipl_team_standings;
select * from ipl_team;*/

select a.team_id 'Team Id', b.team_name 'Team Name', 
(select sum(matches_played) from ipl_team_standings a where a.team_id = b.team_id) as 'Matches Played',
(select sum(matches_won) from ipl_team_standings a where a.team_id = b.team_id) as 'Matches Won',
(select sum(matches_lost) from ipl_team_standings a where a.team_id = b.team_id) as 'Matches Lost'
from ipl_team_standings a join ipl_team b
on a.team_id = b.team_id
group by a.team_id;

-- Have used the aggregate funtion sum() inorder to get the desired output

-- 7) The the bowlers for Mumbai Indians team.
/*select * from ipl_team_players order by player_id;
select * from ipl_player;*/

select b.player_id 'Player Id', a.player_name 'Player Name'
from ipl_team_players b join ipl_player a
on b.player_id = a.player_id 
where b.remarks like '%MI' and b.player_role like '%bowler%'
order by b.player_id;

-- Have used the wildcard funtion 'like' inorder to identify MI team and used a simple join inorder to get the plater details

-- 8) Finding ounders are there in each team and displaying the teams with more than 4 all-rounder in descending order.
/*select * from ipl_team_players;
select * from ipl_team;
select * from ipl_player;*/


select a.team_id , b.team_name, count(a.team_id) as 'Number of all rounders'
...
-- Used a simple join along with the HAVING function inorder to filter and sort the aggregate function count(). This enabled me to get the desired outcome with the aid of a simple join and select function.